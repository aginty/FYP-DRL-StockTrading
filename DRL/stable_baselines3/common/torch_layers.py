...
class CustomExtractor(nn.Module):
    ...
    def __init__(
        ...
        if actor_network is None:
            self.policy_net, self.latent_dim_pi = self.build_mlp_net(net_arch, activation_fn, network="pi")
            
        else:
            self.policy_net = actor_network.to(device="cuda")
            self.latent_dim_pi = self.policy_net.out_layer.out_features
        
        if critic_network is None:
            self.value_net, self.latent_dim_vf = self.build_mlp_net(net_arch, activation_fn, network="vf")
            
        else:
            self.value_net = critic_network.to(device="cuda")
            self.latent_dim_vf = self.value_net.out_layer.out_features
            
    
    def build_mlp_net(self, net_arch, activation_fn, network):
        if isinstance(net_arch, dict):
            network_layers_dims = net_arch.get(network, [])
        else:
            network_layers_dims = net_arch
            
        out_net = []
        last_layer_dim_pi = self.feature_dim
        for curr_layer_dim in network_layers_dims:
            out_net.append(nn.Linear(last_layer_dim_pi, curr_layer_dim))
            out_net.append(activation_fn())
            last_layer_dim_pi = curr_layer_dim
        
        return nn.Sequential(*out_net).to(self.device), curr_layer_dim
